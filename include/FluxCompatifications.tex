...
What we mainly are interested in are the possibility of finding $H$-structures for some some closed subgroup $H\subset G$. An principal bundle $(P,\pi,\mathcal{M},G)$ has a $H$-structure if it is possible to choose frames in $P$ such that the structure group reduces to $H$. A typical example of this is the $O(d)\subset GL(d)$ structure on Riemannian manifolds, which due to the presence of a metric we can consider orthonormal frames $\tilde{e}_a$ such that 
\begin{equation}
    g(\tilde{e}_a,\tilde{e}_b) = \delta_{ab},
\end{equation}
with $g$ the metric. This defines an $O(d)$ structure since on overlapping patches orthonormal frames are related by $O(d)$ transformations. The importance of this in what will follows is the fact that finding a reduced structure is equivalent to the presence of a globally defined non-vanishing tensor on $\mathcal{M}$ that is invariant under some subgroup $H\subset G$. Since it is globally defined we can choose a frame $\{e_{\alpha}\}_{a=1,2,\ldots d}$ such that the tensor is constant on overlapping patches which effectively reduce $GL(d)$ to $H$. 
...
\subsection{Generalized geometry}
...
The infinitesimal action of a generalised diffeomorphism parameterised acting by $O(d,d)\times \mathbb{R}^+$ is parameterised by elements in $Y=w+\rho\in\Gamma(E)$ using the generalised Lie derivative on $X=x+\lambda\Gamma(E)$ as 
\begin{equation}
...
There is a natural action of $E_{d(d)}\times\mathbb{R}^+$, given in \cite{Ashmore:2015joa}, on a section $X\in\Gamma(E)$ as $E' = W\cdot E$ which in components is given by 
\begin{equation}
    \begin{aligned}
...
\section{Generalized structure\label{sec:GeneralisedStructures}}
...
As such we decompose the adjoint representation $\mathbf{133}$ of $\mathfrak{e}_{7(7)}$ under $SU(2)\times SO^*(12)\subset E_{7(7)}$, with $SO^*(12)$ a specific real form,
\begin{equation}
...
\begin{equation}\label{eq:CompatibilityVectorHyperStructure}
    \begin{aligned}
        ...
        \frac{1}{2}\ii s(X,X^*) = \rho^2\qquad &\Longleftrightarrow\qquad \kappa(J_\alpha,J_\beta) = -2\sqrt{q(V)}\delta_{\alpha\beta}
    \end{aligned}
\end{equation}
...
\end{equation}
and we thus find that $\zeta$ is closed and we thus take $\zeta=\dd y$, with $y$ being the coordinate along $S^1$. It then follows from $\dd(\zeta\wedge\Omega)=0$ and $\iota_{\zeta^{\#}}\Omega=0$ that $\Omega$ is closed assuming $\Omega$ does not depend on the coordinate along $S^1$. The last condition from the triplet of momentum maps is the following 
\begin{equation}
    \begin{aligned}
