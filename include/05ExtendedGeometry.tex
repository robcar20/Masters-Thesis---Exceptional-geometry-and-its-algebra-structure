...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
where we used the decomposition of $|q\rangle$ and that $\lambda$ is a highest weight, i.e.\ it is annihilated by all the positive roots $\alpha\in\Delta^+$. This equation is linear in $q$ so consider a single $k$ component, then this can vanish only if $e_\alpha|q\rangle_k= c_ke_{-\alpha}|\lambda\rangle$ with a constant $c_k$ and for some $\alpha\in\Delta^+$ such that 
\begin{equation}
...
\end{equation}
with $\alpha$ a simple root \todo{Why simple?}. Moreover, any other positive root $\tilde{\alpha}\neq\alpha$ has to satisfy 
\begin{equation}
...
The relevant part $\frac{1+\sigma}{2}S^\alpha$ can then be conveniently rewritten in index-free notation as\todo{Why differ from extended geometries}
...
For sufficienct assume that $\lambda=\Lambda_j$ a fundamental weight. In this case we have 
\begin{equation}
...
We have thus shown that ancillary transformations \eqref{eq:AncillaryTransformation} vanish if and only if the highest weight of the coordinate module is a fundamental weight and the corresponding Coxeter label is equal to one. Such modules are easily found for finite dimensional simple Lie algebras and can be found in e.g.\ \cite{Fuchs1997}. For $\mathfrak{a}_n$ this includes any $\Lambda_i$ all other examples are given in \figref{fig:DynkinNoAncillary}. Infinite-dimensional algebras all have $c_i\geq 1$ and therefore will always have ancillary transformations. 
